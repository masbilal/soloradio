<a href="<?=site_url("ngadimin/tambahpost")?>" class="btn btn-warning float-right"><i class="la la-plus-circle"></i> Post Baru</a>
...
<script type="text/javascript">
	$(function(){
		$(".card-body").load("<?=site_url("ngadimin/postupdate?load=true")?>");
	});
	
	function refreshTabel(page){
		$(".card-body").load("<?=site_url("ngadimin/postupdate?load=true&page=")?>"+page);
	}
				...
				$.post("<?=site_url("ngadimin/hapuspostingan")?>",{"theid":id},function(msg){
					...
					if(data.success == true){
						swal.fire("Berhasil","data telah dihapus","success").then((val)=>{
							window.location.href="<?=site_url("ngadimin/postupdate")?>";
						});
					}else{
